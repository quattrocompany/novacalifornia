  ...
  return (
    ...
    >
      <div className={`max-w-[1440px] mx-auto w-full px-4 sm:px-6 md:px-10 flex ${isScrolled ? "items-center" : "items-start"} justify-between pointer-events-auto transition-all`}>
        ...
        {/* MENU DESKTOP (ALINHADO À DIREITA E COM ESPAÇAMENTO AJUSTADO) */}
        <div className="flex items-center justify-end h-10 md:h-12">
          <nav className="hidden md:flex items-center gap-2 lg:gap-3 xl:gap-4 text-xs lg:text-sm tracking-wider">
            <a href="#home" onClick={(e) => scrollToSection(e, "home")} className={`transition-all cursor-pointer ${getLinkClass("home")}`}>
              HOME
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#produto" onClick={(e) => scrollToSection(e, "produto")} className={`transition-all cursor-pointer ${getLinkClass("produto")}`}>
              PRODUTO
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#lazer" onClick={(e) => scrollToSection(e, "lazer")} className={`transition-all cursor-pointer ${getLinkClass("lazer")}`}>
              LAZER
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#plantas" onClick={(e) => scrollToSection(e, "plantas")} className={`transition-all cursor-pointer ${getLinkClass("plantas")}`}>
              PLANTAS
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#localizacao" onClick={(e) => scrollToSection(e, "localizacao")} className={`transition-all cursor-pointer ${getLinkClass("localizacao")}`}>
              LOCALIZAÇÃO
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#realizacao" onClick={(e) => scrollToSection(e, "realizacao")} className={`transition-all cursor-pointer ${getLinkClass("realizacao")}`}>
              REALIZAÇÃO
            </a>
            <span className={getSeparatorClass()}>|</span>

            <a href="#contato" onClick={(e) => scrollToSection(e, "contato")} className={`transition-all cursor-pointer ${getLinkClass("contato")}`}>
              CONTATO
            </a>
          </nav>
